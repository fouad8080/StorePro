          ...
          <div key={index} className="flex items-center justify-between ">
            <div className="flex items-center gap-3 justify-center">
              <img
                src={product.image}
                alt={product.name}
                className="w-10 h-10 rounded-lg object-cover bg-gray-100 dark:bg-[#1B212C]"
              />
              <span className="text-sm text-gray-700 dark:text-gray-300">{product.name}</span>
            </div>

            <div className="flex items-center gap-8 text-sm">
              <span className="text-gray-700 dark:text-gray-300 w-20">{product.price}</span>
              <span className="text-green-600 w-16">↑ {product.growth}</span>
              <span className="text-gray-500 dark:text-gray-400 dark:text-gray-500 w-20 text-right">{product.units}</span>
            </div>
          </div>
        ))}
      </div>
